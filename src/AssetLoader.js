...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  const playerSpriteSheet =
    tuningDoc?.player?.spriteSheet || "assets/MiniThief.png"; // [2]
  const playerImg = await loadImageAsync(playerSpriteSheet);
  const boarSpriteSheet =
    tuningDoc?.boar?.spriteSheet || "assets/MiniSuspiciousMerchantB.png"; // [2]
  const boarImg = await loadImageAsync(boarSpriteSheet);
  const leafImg = await loadImageAsync("assets/Apple.png"); // [3]
  ...
  let playerAnis = buildAnis(
    tuningDoc?.player?.animations ?? tuningDoc?.player?.anims,
    defaultPlayerAnis(),
    {
  ...
  let boarAnis = buildAnis(
    tuningDoc?.boar?.animations ?? tuningDoc?.boar?.anims,
    defaultBoarAnis(),
    {
      spriteSheet: boarImg,
    },
...
export function loadImageAsync(path) {
  if (!path) {
...
async function loadBackgrounds(levelPkg) {
  ...
  return {
    bgFar: await loadImageAsync("assets/background1.png"), // [4]
    bgMid: await loadImageAsync("assets/background2.png"), // [4]
    bgFore: await loadImageAsync("assets/background4b.png"), // [4]
  };
}
